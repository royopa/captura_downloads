...
def parse_titpub(text, buf):
    ...
    date_str = datetime.strptime(tit['data_referencia'], '%Y%m%d').strftime(
        '%Y-%m-%d'
    )
    return date_str
...
def parse_vnataxatitpub(text, buf):
    ...
    writer.writerow(
        [pp.parse(date), 'IPCA', pp.parse(value), projecao, pp.parse(vigencia)]
    )
    ...
    vigencia = root.cssselect(sel)[0].text_content()
    writer.writerow(
        [pp.parse(date), 'IGPM', pp.parse(value), projecao, pp.parse(vigencia)]
    )
