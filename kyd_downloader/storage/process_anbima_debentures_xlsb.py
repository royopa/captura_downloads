...
blobs = list(
    storage_client.list_blobs(bucket, prefix='DADOS DIÁRIOS - ANBIMA - 2021')
)
...
def to_date_str(x):
    f = lambda x: (datetime(1900, 1, 1) + timedelta(x - 2)).strftime(
        '%Y-%m-%d'
    )
    try:
        return f(int(x))
...
for blob in blobs:
    ...
    _df = pd.read_excel(
        tempf, engine='pyxlsb', sheet_name='ANBIMA - DEBÊNTURES'
    )
    ...
    if (
        df['refdate'].iloc[0] >= '2021-04-20'
        or df['refdate'].iloc[0] < '2019-03-19'
    ):
        continue

    df['ask_yield'] = pd.to_numeric(df['ask_yield'], errors='coerce').astype(
        'float64'
    )
    df['bid_yield'] = pd.to_numeric(df['bid_yield'], errors='coerce').astype(
        'float64'
    )
    df['ref_yield'] = pd.to_numeric(df['ref_yield'], errors='coerce').astype(
        'float64'
    )
